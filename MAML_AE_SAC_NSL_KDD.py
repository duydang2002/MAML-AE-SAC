...
class data_cls:
    def __init__(self, train_test, **kwargs):
        ...
        self.train_path = kwargs.get('train_path', 'C:/Users/duyda/Documents/MAML/data/NSL-KDD/KDDTrain+.txt')
        self.test_path = kwargs.get('test_path', 'C:/Users/duyda/Documents/MAML/data/NSL-KDD/KDDTest+.txt')
        ...
        if not formated:
            ...
            if self.train_test == "train":
                train_df = self.df[test_indx:self.df.shape[0]]
                x = train_df.drop('labels', axis=1)
                y = train_df['labels']
                
                class_counts = y.value_counts().to_dict()   
                
                print("Số lượng mẫu ban đầu theo nhãn chi tiết:")
                ...
                for cls, count in class_counts.items():
                    if count < t:
                        ...
                        sampling_strategy[cls] = number_sample  
                    else:
                        sampling_strategy[cls] = count  # Keep original count

                
                smote = SMOTE(sampling_strategy=sampling_strategy,random_state=42, k_neighbors=t-1)
                X_smote, y_smote = smote.fit_resample(x, y)

                print("Số lượng mẫu sau SMOTE theo nhãn chi tiết:")
                print(pd.Series(y_smote).value_counts())

                # Combine resampled data 
                df_smote = pd.concat([X_smote, y_smote.rename('labels')], axis=1)
            frames2 = [self.df[:test_indx], df_smote]
            self.df = pd.concat(frames2)
            self.df = pd.concat([self.df.drop('labels', axis=1),
                    pd.get_dummies(self.df['labels'])], axis=1)
            
            # Save data
            test_df = self.df.iloc[:test_indx]
            test_df = shuffle(test_df, random_state=np.random.randint(0, 100))
            self.df = self.df[test_indx:self.df.shape[0]]
...
class QNetwork():
    ...
    def __init__(self, obs_size, num_actions, hidden_size=100,
                 hidden_layers=1, learning_rate=.002, net_name='qv'):
        """
...
class Agent(object):
    ...
    def init_target_network(self, K,H):  
        ...
        self.target_actor_network.compile(optimizer=optimizers.Adam(lr),
                               loss=sac_loss)  
        ...
        self.target_q0_network.compile(optimizer=optimizers.Adam(lr),
        ...
        self.target_q1_network.compile(optimizer=optimizers.Adam(lr),
                               loss=tf.keras.losses.mse)  
        ...
        self.v_network_target.compile(optimizer=optimizers.Adam(lr),
                               loss=tf.keras.losses.mse) 
        ...
        self.target_v_network_target.compile(optimizer=optimizers.Adam(lr),
                               loss=tf.keras.losses.mse)  
    ...
    def meta_train(self, states, actions, next_states, def_reward, dones, K):
        ...
        self.update_target_net(self.target_v_network_target,
                           self.v_network_target, self.net_learning_rate)
        return loss_actor
...
class DefenderAgent(Agent):
    ...
    def act(self, states, batch=False):
        ...
        if batch == False:
            ...
            probs = self.actor_network.model.predict(states, verbose = 0) 
            actions = np.array([np.random.choice(self.actions, p=prob) for prob in probs])
        return actions
...
class RLenv(data_cls):
    ...
    def act(self, defender_actions, attack_actions, batch=False):
        ...
        attack_actions = attacker_agent.act(self.states,batch)
    ...
    def get_states(self, attacker_actions, batch =False):
        first = True
        for attack in attacker_actions:
            if first:
                ...
                minibatch = pd.concat([minibatch, self.df[self.df[self.attack_names[attack]] == 1].sample(1)] , ignore_index=True)
...
if __name__ == "__main__":
    ...
    kdd_train = "C:/Users/duyda/Documents/MAML/data/NSL-KDD/KDDTrain+.txt"
    kdd_test = "C:/Users/duyda/Documents/MAML/data/NSL-KDD/KDDTest+.txt"

    formated_train_path = "formated_train_smote.data"
    formated_test_path = "formated_test_smote.data"
    ...
    for epoch in range(num_episodes):
        ...
        for i_iteration in range(iterations_episode):
            ...
            attack = [env.attack_types.index(env.attack_map[env.attack_names[att]]) for att in attack_actions]
        ...
        print("|Def Estimated: {}| Att Labels: {}".format(env.def_estimated_labels,
                                                          env.def_true_labels))
        log_text += (
            f"\r\n|Epoch {epoch:03d}/{num_episodes:03d}| time: {end_time - start_time:2.2f}|"
            ...
            f"|Def Estimated: {env.def_estimated_labels}| Att Labels: {env.def_true_labels}\n"
        )
    ...
    defender_agent.actor_network.model.save_weights("models/smote_model2.weights.h5", overwrite=True)
    with open("models/smote_model2.json", "w") as outfile:
        json.dump(defender_agent.actor_network.model.to_json(), outfile)

    formated_test_path = "formated_test_adv_origin.data"

    with open("models/smote_model2.json", "r") as jfile:
        model = model_from_json(json.load(jfile))
    model.load_weights("models/smote_model2.weights.h5")
    ...
    log += f"Smote Def = {def_hidden_layers} Layers and {def_hidden_size} each, ATK= {att_hidden_layers} Layers and {att_hidden_size} each  "
    log += f'Net_lr = {net_lr}, Gamma = {gamma}, alpha = {alpha}  {num_episodes} lr= {lr_meta} ep, K={K}, H={H} {number_sample} , t = {t} \n minibatchsize = {minibatch_size} '
    log += "\n" + outputs_df.to_string() + "\n"
    log += 'Performance measures on Test data\n'
    log += 'Accuracy =  {:.4f}\n'.format(accuracy_score(aggregated_data_test, actions))
    log += 'F1 =  {:.4f}\n'.format(f1_score(aggregated_data_test, actions, average='weighted'))
    log += 'Precision_score =  {:.4f}\n'.format(precision_score(aggregated_data_test, actions, average='weighted'))
    log += 'recall_score =  {:.4f}\n'.format(recall_score(aggregated_data_test, actions, average='weighted'))
    ...
    plt.savefig(f'{accuracy_score(aggregated_data_test, actions)}confusion_matrix_adversarial.png',  dpi=1000)
